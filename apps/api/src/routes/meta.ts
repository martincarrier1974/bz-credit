...
metaRouter.get('/', async (_req, res) => {
  try {
    ...
    emp.sort((a: { name: string }, b: { name: string }) => a.name.localeCompare(b.name));
    ...
    gl.sort((a: { company?: string; code: string }, b: { company?: string; code: string }) =>
    ...
    sup.sort((a: { name: string }, b: { name: string }) => a.name.localeCompare(b.name));
    return res.json({
      employees: emp,
